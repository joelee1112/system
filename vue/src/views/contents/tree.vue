  ...
  mounted() {
    ...
    $(this.$el)
        ...
        contextmenu: {
          items: function(node) {
            ...
            tmp.create.action = data => {
              ...
              inst.create_node(obj, { type: "content" });
            };
            tmp.edit = {
              label: "設定",
      ...
      .on("create_node.jstree", (a, b) => {
        var parent = b.instance.get_node(b.parent);
        console.log(b);
      ...
      .on("rename_node.jstree", (a, b) => {
        let title = b.node.text;
        let data = { title };
        contents.rename({ id: b.node.id }, data);
      })
      .on("delete_node.jstree", (a, b) => {
        contents.delete({ id: b.node.id });
      })
      .on("move_node.jstree", function(e, data) {
        console.log(data.parent);
        var parent = data.instance.get_node(data.parent);
        console.log(parent.original.id);
